...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("perkcoin"))
        return false;
...
{
    // Convert perkcoin:// to perkcoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("perkcoin://"))
    {
        uri.replace(0, 12, "perkcoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Perkcoin.lnk";
}
...
{
    return GetAutostartDir() / "perkcoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Perkcoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  perkcoin-qt [" + tr("command-line options") + "]                     " + "\n";
